...
- **ProviderClient**: `densify` returns `DensificationResult(content: String, title: String?)`. `requestDensification` runs the CLI with schema (Codex/Claude) or prompt-only (Gemini) and parses response for both `content` and `title`. Content keys include `content`, `result`, `response`, `output`, `message`, `text`.
